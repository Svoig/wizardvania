...
function generateBragText({ tookDamage, defeatedEnemy, collectedItem, retried }) {
    const bragIntros = [
        `As I stepped over the threshold into the chamber,\nI was filled with complete confidence\nin my exemplary skills and finesse.\n`,
    ...
    const tookDamageBrags = [
        `I was invincible! None of the nightmarish\ndangers even came close to me.\n`,
        `I skillfully avoided every danger with,\nif I may say so, poise and elegance.\n`,
        `The dangers swarmed around me, but\nnothing could so much as lay a finger on me!\n`
    ];

    const defeatedNoEnemiesBrags = [
        `Every foul, slobbering monster fell\nbefore my might.\n`,
        `Not one soulless demon was left standing by\nthe time I approached the exit.\n`,
        `My unparalleled skill and power made short work of\nthe horde of monsters lurking here.\n`
    ];

    const collectedNoItemBrags = [
        `I found treasures in that room you\ncouldn't even imagine!\n`,
        `This chamber was an absolute treasure trove! I could hardly carry it all!\n`,
        `I found enough treasure in that room to buy the\nwhole town twice over!\n`
    ];

    const concludingBrags = [
        `Having risked life and limb, I am one\nstep closer...\nthat treasure is mine!`,
        `My life was hanging by a thread, but nothing\ncould sway me from my quest!`,
        `Another exhilirating challenge, but of\ncourse it was no match for me!`
    ];
...
function sceneSetup({
    ...
    every("arachnos", (arachnos) => {
        ...
        arachnos.on("destroy", () => {
            ...
            maxHealthUp.collides("player", () => {
                ...
                print("Your max health has increased by one!", 5);
            ...
            maxBoostUp.collides("player", () => {
                ...
                print("Maximum boosts increased by one!", 5);
    ...
    every("flameBatSwarm", (swarm) => {
        ...
        on("destroy", "flameBat", (bat) => {
            ...
            if (bats.length === 0) {
                const collectedRewards = [];
                const maxHealthUp = add([...maxHealthUpComponents, pos((map.width() / 2) + 20, 120)]);
                const maxBoostUp = add([...maxBoostUpComponents, pos((map.width() / 2) - 20, 120)]);
                const fireGun = add([rect(TILE_UNIT * 2, TILE_UNIT), FIREBALL_COLOR, pos((map.width() / 2) - 60, 120), "fireGun"]);
                ...
                maxBoostUp.collides("player", () => {
                    ...
                    // const hintBox = addHintBox(map);
                    // const hintText = addHintText("You can now boost an additional time\nbefore hitting the ground!", map);
                    print("Your max number of boosts has increased by one!", 5);
    ...
    player.collides("spike", (spike) => {
        if (Math.round(rand(0, 100) % 3 === 0)) {
            print("THIEF: Ouch!! ...Nobody saw that.", 5);
        }
    ...
    player.collides("earthSpike", (spike) => {
        if (Math.round(rand(0, 100) % 3 === 0)) {
            print("THIEF: Agrrghh! - I didn't even feel that!", 5);
        }
        PLAYER_STATE.health -= spike.strength;
        healthText.text = getPlayerHealth();
    });
    ...
    player.collides("enemy", (enemy) => {
        if (Math.round(rand(0, 100) % 3 === 0)) {
            ...
            print("THIEF: OWW! - Hey, you almost got me!", 5);
        }
        healthText.text = getPlayerHealth();
    ...
    player.collides("ENEMY_BULLET", (bullet) => {
        if (Math.round(rand(0, 100) % 3 === 0)) {
            print("THIEF: Ahhh, my eye! - I mean...\nbetter luck next time!", 5);
        }
    ...
    keyPress(AMMO_CYCLE_BACK, () => {
        if (PLAYER_STATE.availableAmmos.length > 1) {
            ...
            const nextIndex = currentIndex - 1 >= 0 ? currentIndex - 1 : PLAYER_STATE.availableAmmos.length - 1;
...
scene("seven", () => {
    add([text("To be continued!\n\n\n\nCheck back soon for more levels!", 12), pos(25, 25)]);
});
...
scene("interlude", ({ currentLevel, previousElement, nextLevel, tookDamage, defeatedEnemy, collectedItem, retried }) => {
    ...
    add([
        text("Press ENTER to continue", 10),
        pos(10, 250)
    ]);
